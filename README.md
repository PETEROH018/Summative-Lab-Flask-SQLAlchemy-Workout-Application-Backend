...
## Description
- This is a workout application that is used to assign exercises to workouts thus creating workoutexercises
...
- A single exercise can be in multiple workouts and includes details about the exercise name, exercise category and a boolean value of whether equipment is needed in an exercise (equipment_needed)
...
- This project uses Alembic that is wrapped in Flask-Migrate for tracking the database versions allowing upgrading and rolling back of any changes made to the database schema
...
- Seeding the database is generating sample data in the database for use during development and testing
...
- To do a POST request for a workout exercise for a workout with id 1 and an exercise with id 1, select POST method on POSTMAN and use the URL 'http://127.0.0.1:5000/workouts/1/exercises/1/workout_exercises'. Then specify the number of repetitions(reps),sets and the duration in seconds of the workout exercise
